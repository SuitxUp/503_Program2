...
class Shop_org
{
   public:
      ...
      Shop_org(int num_chairs) : max_waiting_cust_((num_chairs > 0 ) ? num_chairs :
      kDefaultNumChairs), customer_in_chair_(0),
      in_service_(false), money_paid_(false), cust_drops_(0)
      {
         init();
      };

      Shop_org() : max_waiting_cust_(kDefaultNumChairs), customer_in_chair_(0),
      in_service_(false),
